...
/**
 * 관리자용 사용자 관리 서비스
 * 사용자 생성, 조회, 수정, 삭제 및 통계 기능을 제공
 */
public interface AdminUserService {

    /**
     * 계정 생성
     * 관리자가 직접 계정을 생성하고 임시 비밀번호를 발급
     * 생성된 계정 정보는 이메일로 전송
     *
     ...
     */
    AdminUserResponse.UserCreated createUser(AdminUserRequest.Create request, UserRole role);

    /**
     * 전체 사용자 목록 조회
     * 페이지네이션, 필터링, 검색을 지원
     * 역할(강사/학생/관리자), 기간, 계정 상태별 필터링이 가능
     *
     ...
     */
    AdminPageResponse<AdminUserResponse.UserListItem> getUserList(
     ...
     * 사용자 상세 정보 조회
     * 특정 사용자의 상세 정보를 조회
     * 기본 정보, 최근 로그인 히스토리, 참여 클래스 정보를 포함
     *
     ...
     * @throws CustomException USER_NOT_FOUND - 사용자를 찾을 수 없는 경우
     */
    List<AdminUserResponse.UserHistory> getUserHistory(Long userId, int limit);

    /**
     * 사용자 계정 삭제 (Soft Delete)
     * 사용자 계정을 비활성화 처리
     * 실제 데이터는 삭제되지 않고 상태만 변경
     *
