...
public interface MypageClassFileService {
    ...
    /** 클래스 자료 파일 업로드 */
    List<MypageClassFileResponse> uploadFiles(MypageClassFileUploadRequest rq) throws Exception;

    /** 클래스 ID 기준으로 자료 파일 목록 조회 */
    Page<MypageClassFileResponse> getFilesByClassId(Long classId, Pageable pageable);

    /** 특정 수업 날짜 기준으로 자료 파일 조회 */
    Page<MypageClassFileResponse> getFilesByLectureDate(Long classId, LocalDate lectureDate, Pageable pageable);
